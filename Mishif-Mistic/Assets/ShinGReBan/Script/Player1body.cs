...
public class Player1body : MonoBehaviour
{
    ...
    private Vector2 Main = new Vector2(-482f, 152f);

    //0 : 動物画像を透明にする基準座標
    //1 : 動物画像を逆の端に移動する基準座標
    private Vector2 Up0 = new Vector2(-482f, -49f);
    private Vector2 Up1 = new Vector2(-482f, -250f);
    private Vector2 Down0 = new Vector2(-482f, 353f);
    private Vector2 Down1 = new Vector2(-482f, 554f);
    ...
    */
    ...
    public void SetLevelThree()
    {
        for (int i = 0; i < Animaru.Length; i++)
        {
            ...
            AnimaruRect[i].localPosition = new Vector2(-482f, 420f - i * 134);
        ...
        if (SlideFlag)
        {
            ...
            for (int i = 0; i < Animaru.Length; i++)
            {
                ...
                if (AnimaruRect[i].localPosition.y == Main.y)
                {
                     ...
                     */
                    if (AnimaruImage[i].sprite == TortoiseSprite)
                    {
                        WeponType = 0;
                    }
                    else if (AnimaruImage[i].sprite == ScorpionSprite)
                    {
                        WeponType = 1;
                    }
                    else
