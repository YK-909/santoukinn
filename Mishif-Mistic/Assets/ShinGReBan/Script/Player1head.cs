...
public class Player1head : MonoBehaviour
{
    ...
    private Vector2 Main = new Vector2(-701f, 152f);

    //0 : 動物画像を透明にする基準座標
    //1 : 動物画像を逆の端に移動する基準座標
    private Vector2 Up0 = new Vector2(-701f, -49f);
    private Vector2 Up1 = new Vector2(-701f, -250f);
    private Vector2 Down0 = new Vector2(-701f, 353f);
    private Vector2 Down1 = new Vector2(-701f, 554f);
    ...
    */
    ...
    public void SetLevelThree()
    {
        for (int i = 0; i < Animaru.Length; i++)
        {
            ...
            AnimaruRect[i].localPosition = new Vector2(-701f, 420f - i * 134);
            ...
            {
                case 1:
                    AnimaruImage[i].sprite = FrogSprite;
                    break;
                case 2:
                    AnimaruImage[i].sprite = FlyPanSprite;
                    break;
                case 3:
                    AnimaruImage[i].sprite = LionSprite;
                    break;
                case 4:
        ...
        if (SlideFlag)
        {
            ...
            for (int i = 0; i < Animaru.Length; i++)
            {
                ...
                if (AnimaruRect[i].localPosition.y == Main.y)
                {
                     ...
                     */
                    if (AnimaruImage[i].sprite == FrogSprite)
                    {
                        WeponType = 0;
                    }
                    else if (AnimaruImage[i].sprite == LionSprite)
                    {
                        WeponType = 1;
                    }
                    else
    ...
    void Update()
    {
        if (_flag)
        {
            if (Input.GetKeyDown(KeyCode.Joystick2Button2) && !SlideFlag)
            {
                SlideFlag = true;
                PosFlag = true;
                Dir = 0;
            ...
            if (Input.GetKeyDown(KeyCode.Joystick2Button1) && !SlideFlag)
            {
                SlideFlag = true;
                PosFlag = true;
                Dir = 1;
