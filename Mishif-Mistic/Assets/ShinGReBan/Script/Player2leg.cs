...
public class Player2leg : MonoBehaviour
{
    //各部分のスプライト
    [SerializeField]
    ...
    //0 : 動物画像を透明にする基準座標
    //1 : 動物画像を逆の端に移動する基準座標
    private Vector2 Up0 = new Vector2(256f, -20f);
    private Vector2 Up1 = new Vector2(256f, -101f);
    private Vector2 Down0 = new Vector2(256f, 142f);
    private Vector2 Down1 = new Vector2(256f, 223f);
    ...
    */
    ...
    public void SetLevelThree()
    {
        for (int i = 0; i < Animaru.Length; i++)
        {
            ...
            AnimaruRect[i].localPosition = new Vector2(256f, 169f - i * 54);
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Z) && !SlideFlag)
        {
            SlideFlag = true;
            PosFlag = true;
            Dir = 0;
        }
        if (Input.GetKeyDown(KeyCode.X) && !SlideFlag)
        {
            SlideFlag = true;
            PosFlag = true;
            Dir = 1;
